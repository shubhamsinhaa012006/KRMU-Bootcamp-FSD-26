...
export const PlaylistDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        // Display the actual error message from Spotify
        setSearchError(error.message.replace('API error: ', ''));
      } finally {
  ...
  // Derive mood and songs safely (avoids hook-ordering errors when playlist is null)
  const moodObj = useMemo(() => {
    if (!playlist) return MOODS[0];
    return MOODS.find(m => m.name === playlist.mood) || MOODS[0];
  }, [playlist]);

  const currentSongs = useMemo(() => playlist?.songs || [], [playlist]);

  const totalDuration = useMemo(
    () => currentSongs.reduce((acc, curr) => acc + (curr?.duration || 0), 0),
    [currentSongs]
  );
  ...
  const handleDragEnd = (event) => {
    ...
    updatePlaylistSongs(playlist.id, arrayMove(currentSongs, oldIndex, newIndex));
  };

  const removeSong = (songId) => {
    updatePlaylistSongs(playlist.id, currentSongs.filter(s => s.id !== songId));
  };
      ...
      <div className="flex flex-col gap-6 mt-4">
        ...
        {currentSongs.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-white/10 rounded-3xl text-neutral-500">
            ...
            <button onClick={() => setIsAddModalOpen(true)} className="text-primary hover:underline">
            ...
            >
              <div className="p-6 border-b border-white/10 flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Search className="text-primary" size={22} /> Search Spotify
                  </h2>
                  <button onClick={closeAddModal} className="px-4 py-2 text-neutral-400 hover:text-white transition-colors">
                    Close
                  </button>
                </div>
                <input
              ...
              <div className="p-6 overflow-y-auto flex flex-col gap-4 flex-1">
                {isSearching ? (
                  ...
                  <p className="text-center text-workout py-10">{searchError}</p>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between p-4 rounded-xl border transition-colors ${
                          isAdded ? 'border-primary/30 bg-primary/5' : 'border-white/5 hover:bg-white/5'
                        }`}
                      >
                        <div className="flex items-center gap-4 flex-1 overflow-hidden">
                          {song.coverUrl ? (
                            <img src={song.coverUrl} alt={song.album} className="w-12 h-12 rounded object-cover shadow-lg" />
                          ) : (
